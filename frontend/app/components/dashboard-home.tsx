...
export default function DashboardHome() {
  ...
  return (
    <AppShell
      user={user ? { name: user.name || user.email, email: user.email, avatarUrl: user.avatar_url } : undefined}
      active={tab}
    ...
    >
      <section className="content-wrap">
        <div className="page-heading">
          ...
          <button className="primary-button" onClick={() => setShowCompose(true)}>
        ...
        <div className="metric-strip">
          <div>
            ...
            <strong>{tab === 'scheduled' ? 'Queued sends' : 'Delivery history'}</strong>
          </div>
          <div>
            <span className="metric-label">Showing</span>
            <strong>{loading ? '...' : `${emails.length} emails`}</strong>
          </div>
          <div>
            <span className="metric-label">Transport</span>
            <strong>Ethereal SMTP</strong>
          </div>
        </div>
        ...
        <section className="activity-panel">
          <div className="panel-header">
            ...
            <button className="refresh-button" onClick={() => loadEmails()} title="Refresh activity">
              Refresh
            </button>
          </div>
          ...
          {loading ? (
            <EmptyState loading title="" detail="" />
          ) : emails.length === 0 ? (
              ...
              <table>
                <thead>
                  <tr>
                    <th>Recipient</th>
                    <th>Subject</th>
                    <th>{tab === 'scheduled' ? 'Scheduled for' : 'Sent at'}</th>
                    <th>Status</th>
                  </tr>
                </thead>
                <tbody>
                  {emails.map((email) => (
                    <tr key={email.id}>
                      <td className="recipient">{email.recipient_email}</td>
                      <td>{email.subject}</td>
                      <td>{formatDate(tab === 'scheduled' ? email.scheduled_at : email.sent_at)}</td>
                      <td>
      ...
      {showCompose && (
        <ComposeModal
          ...
          onCreated={() => {
            ...
            setNotice('Campaign scheduled successfully');
          }}
        />
      )}
    </AppShell>
...
function ComposeModal({ senders, onClose, onCreated }: { senders: Sender[]; onClose: () => void; onCreated: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!file) {
      setError('Choose a CSV file with an email column');
      return;
    }
    form.set('leads', file);
    setSubmitting(true);
    ...
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="compose-modal">
        <div className="modal-heading">
          <div>
            <p className="eyebrow">New outbound</p>
        ...
        <form onSubmit={submit}>
          <label>
            Subject
            <input name="subject" required placeholder="A useful note for your leads" />
          </label>
          <label>
            Message
            <textarea name="body" required rows={5} placeholder="Write the message your recipients will receive." />
          </label>
          <div className="form-grid">
            <label>
              Start time
              <input name="start_time" type="datetime-local" required defaultValue={new Date(Date.now() + 5 * 60 * 1000).toISOString().slice(0, 16)} />
            </label>
            <label>
              Delay (ms)
              <input name="delay_ms" type="number" min="0" defaultValue="1000" />
            </label>
            <label>
              Hourly limit
              <input name="hourly_limit" type="number" min="1" defaultValue="100" />
            </label>
            <label>
              Sender
              <select name="sender_id" defaultValue={senders[0]?.id || ''} disabled={!senders.length}>
                {senders.length ? senders.map((sender) => <option key={sender.id} value={sender.id}>{sender.name}</option>) : <option>No sender configured</option>}
              </select>
            </label>
          </div>
          <label className="file-drop">
            Lead CSV
            <input type="file" accept=".csv,text/csv" onChange={chooseFile} required />
            <span>{file ? `${file.name} · ${Math.max(0, leadCount)} leads detected` : 'Choose a CSV with an email column'}</span>
          </label>
          ...
          <div className="modal-actions">
            ...
            <button className="primary-button" disabled={submitting || !senders.length}>
